...
def page1():
    ...
    if st.checkbox('Показать инструкцию', value = True):
        ...
        st.write("""2. Так же должен построиться график целевой функции с критическими точками, если кол-во переменных <= 2, """)
...
def page2():
    ...
    if st.checkbox('Показать инструкцию', value = True):
        ...
        st.header('Интерпретация графиков')
        st.write("""1. На первом графике (Задание 3) изображена целевая функция, а так же точки(значение функции) на каждой итерации""")
    ...
    eps = st.sidebar.number_input('Введите точность',  value=1e-3, min_value=1e-5,max_value=1., step=1e-6, format='%.6f')
...
def page3():
    ...
    if st.checkbox('Показать инструкцию', value = True):
        ...
        st.write("""2. Выбрать нужный метод""")
        st.write("""3. Ввести список координат начальной точки. Размер вектора начальной точки зависит от кол-ва переменных""")
...
def page4():
    ...
    if st.checkbox('Показать инструкцию', value = True):
        ...
        st.write("""1. Если кол-во признаков = 1, то будет построены график рассеивания исходных данных и линия - результат работы модели""")
        st.write("""2. Если кол-во признаков = 2, то будет построены график рассеивания исходных данных и плоскость - результат работы модели""")
...
def page7():
    ...
    if st.checkbox('Показать инструкцию', value = True):
        ...
        st.write("""3. Задать коэффициенты целевой функциив в виде списка: [2,3]""")
        st.write("""4. Задать коэффициенты ограничений в виде массива: [[2,1], [1,4]]""")
        st.write("""5. Задать коэффициенты свободных членов ограничений в виде списка: [10, 13]""")
        st.write("""6. Нажать кнопку Click на боковой панели, результаты программы отобразятся ниже документации""")

        st.header('Интерпретация графиков')
        st.write("""1. На экране отобразятся оптимальная точка и значение функции в ней""")
        st.write("""2. Если кол-во переменных = 2, будет построен графики ограничений, оптимальная точка и контурный график целевой функции""")
